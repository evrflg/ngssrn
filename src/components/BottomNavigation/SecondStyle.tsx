import Background from "@/components/icons/navigation/second/background";
import { useTheme } from "@/hooks/theme/ThemeProvider";
import { type Tab } from "@/types/navigation";
import { resolveSafeAreaExtensionBg } from "@/utils/resolveSafeAreaExtensionBg";
import { getIcon } from "@/utils/navigation/second";
...
const LABEL_FONT_SIZE = rf(12);

const Second = ({ tabs, onNavigate, isTabActive }: BottomNavProps) => {
  const {
    theme,
  ...
  return (
    <View style={styles.container}>
      ...
      <View
        style={styles.row}
        className="w-full flex-1 flex flex-row items-center justify-evenly overflow-visible"
      >
        {tabs.map((tab, index) => {
      ...
      {/* 金色边框 —— 沿弧形路径描边（角标单独再叠一层，否则会盖住 MemberDayFooterBadge） */}
      <View pointerEvents="none" style={styles.borderLayer}>
        <Background fill="none" showBorder />
      </View>

      {/* 会员日角标：在金边层之上；pointerEvents=none 穿透到下层，故可叠在 centerIcon 之上且不挡中间按钮点击 */}
      ...
      {/* 中间凸起图标：叠在 Tab 行之上；会员日在更上层绘制（见 memberDayBadgeLayer） */}
      {CenterIconComp && (
        <View
...
const styles = StyleSheet.create({
  ...
  label: {
    ...
    lineHeight: Math.ceil(LABEL_FONT_SIZE * 1.35),
  },
  borderLayer: {
    position: "absolute",
    top: 0,
